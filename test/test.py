...
def test_graph():
    ...
    assert (
        len(
            set(
                [
                    gm.graph.Graph(G1).hash_edge_attr(ed[0], ed[1], ed[2]["color"])
                    for ed in list(G1.edges(data=True))
                ]
            ).intersection(gm.graph.Graph(G1, edge_attr_key="color").get_edges_hash())
        )
        == 2
    )
    ...
    G1.add_edge("1", "2", color="red")
    G1.add_edge("1", "3", color="green")
    assert gm.graph.Graph(G1, edge_attr_key="color").density() == len(
        G1.edges(data=True)
    )
...
def test_intersect_union():
    ...
    # IF COLOR
    G1 = nx.DiGraph()
    G1.add_node("1", color="blue")
    G2 = nx.DiGraph()
    G2.add_node("1", color="red")

    GM1, GM2 = gm.graph.Graph(G1), gm.graph.Graph(G2)
    assert GM1.size_node_intersect(GM2) == 1
    GM1, GM2 = gm.graph.Graph(G1, node_attr_key="color"), gm.graph.Graph(
        G2, node_attr_key="color"
    )
    ...
    GM1, GM2 = gm.graph.Graph(G1, edge_attr_key="color"), gm.graph.Graph(
        G2, edge_attr_key="color"
    )
